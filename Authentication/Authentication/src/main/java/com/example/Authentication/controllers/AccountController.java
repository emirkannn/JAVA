...
import com.example.Authentication.models.RegisterDto;
...
public class AccountController {
	...
	public String register(Model model, @Valid @ModelAttribute RegisterDto registerDto, BindingResult result) {
		...
		if (!registerDto.getPassword().equals(registerDto.getConfirmPassword())) {
			result.addError(new FieldError("ergisterDto", "confirmPassword",
					"Password and Confirm Password do not match"));
		}
		
		AppUser appUser = repo.findByEmail(registerDto.getEmail());
		if (appUser !=null) {
			result.addError(new FieldError("ergisterDto", "email",
		...
		try {
			...
			result.addError(new FieldError("ergisterDto", "firstName",
					ex.getMessage()));
		}
